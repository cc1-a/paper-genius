...
db.select(orders).filter_by(user_id=g.user.id).order_by(orders.order_date.desc())
).scalars().all()
...
db.select(cart).filter(cart.user_id == g.user.id, cart.id.in_(selected_ids))
).scalars().all()
...
db.select(users).filter((users.email == username_input) | (users.name == username_input))
).scalar_one_or_none()
if user:
